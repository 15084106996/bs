<template>
    <div>
        <div class="teacherHomeworkHeader">
        ...
        <el-dialog
            ...
            <el-progress class="teacherHomework" :text-inside="true" :stroke-width="26" :percentage="73" status="success"></el-progress>
            <span >
                    作业二：
                </span>
            <el-progress class="teacherHomework" :text-inside="true" :stroke-width="24" :percentage="65" status="exception"></el-progress>
            <span >
                    作业三：
                </span>
            <el-progress class="teacherHomework" :text-inside="true" :stroke-width="22" :percentage="80" status="success"></el-progress>
            <span >
                    作业四：
                </span>
            <el-progress class="teacherHomework" :text-inside="true" :stroke-width="20" :percentage="90" status="success"></el-progress>
            <span slot="footer" class="dialog-footer">
    ...
    module.exports= {
        ...
        methods:{
            ...
            currentChange(count){//分页回调
                ...
                this.initteacherHomeworkInfos();
            },
            serchByIdOrName() { // 点击搜索
                this.paramsObj.pageNum = 1;
                this.initteacherHomeworkInfos();
            },
            initteacherHomeworkInfos(){
                getRequest('http://localhost:8080/searchTeacherHomework',this.paramsObj).then(resp=>{
                    if (resp.status==200) {
                        this.teacherHomeworkInfos=resp.data.data;
                        this.total=resp.data.total;
                    }
                })
            },
            initteacherTopicInfos() {
                getRequest('http://localhost:8080/searchStudentTopic',this.paramsObj).then(resp=>{
                    if (resp.status==200) {
                        this.teacherTopicInfos=resp.data.data;
                    }
                })
            }
        },
